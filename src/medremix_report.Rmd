...
library(cowplot)
library(ggthemr)
source('R/medremix_visualizations.R')
ggthemr('fresh')
...
```{r load_data}
medremix_output <- read_tsv('../data/manual/CMP-01-08-cfDNA-1_downsampled_1000.tsv') %>%
  mutate(
    cpg_bin = factor(round(round(cpg_count * 20 / max(cpg_count)) * max(cpg_count) / 20)),
    gc_bin = factor(round(gc_content * 4, 1) / 4)
  )
medremix_model <- readRDS('../data/manual/CMP-01-08-cfDNA-1_model.Rds')
```
...
plot_scatter_bin_methylation(
  medremix_output %>% filter(coverage < 100),
